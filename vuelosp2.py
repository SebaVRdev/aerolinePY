...
#METODO DE COMPRA
def realizarCompra(rutUsuario,nombre,asientosSeleccionados,tel,banco):
    if len(asientosSeleccionados) == 0:
        print("No se seleccionaron asientos")
    else:
        # REALIZAMOS toda la logica de compra
        # RECORREMOS lista de asientos seleccionados y iremos almacenando el valor total
        precio = 0
        for asiento in asientosSeleccionados:
            #DEFINIMOS precio del asiento en base a su posicion
            if asiento > 24 and asiento < 36:
                precio += 240000
            else:
                precio += 78900 
        ...
        # Una vez hayamos calculado el precio total a pagar en base al tipo de asiento que se compro, generamos descuento
        totalDescuento = 0
        ...
        print("Asiento/s: " + str(asientosSeleccionados) + " Precio: " + str(precio)+" Banco del Usuario: "+banco)
        ...
        usuarios[rutUsuario]  = {
            ...
            "pagado:" : totalDescuento
        }

def ValidarAntesdeComprar(numAsiento):
    # REVISAMOS si el numero de asiento esta en la lista de asientos general disponibles
    if numAsiento in listaAsientos:
        # MARCAMOS como ocupado para que si el mismo usuario quiere seguir comprando no pueda elegir 2 veces el mismo asiento
...
while menu != '0' :
    ...
    if menu == '1' :
        ...
        listaAsientosAcomprar = []
        #PREGUNTO si desea seguir comprando mas asientos
        asientoAComprar = int(input("Ingrese un asiento, en caso querer cancelar ingrese '0':\n"))
        while asientoAComprar != 0: 
            if ValidarAntesdeComprar(asientoAComprar):
                listaAsientosAcomprar.append(asientoAComprar)
            else:
                print("EL ASIENTO QUE SE SELECCIONO NO ESTA DISPONIBLE!")
        
            asientoAComprar = int(input("Ingrese otro asiento si lo desea, en caso de no querer mas ingrese '0':\n"))

        realizarCompra(rutPasajero,nombrePasajero,listaAsientosAcomprar,telefonoPasajero,bancoPasajero)
        print(usuarios)
